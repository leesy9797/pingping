...
ReviewInfo reviewInfo = (ReviewInfo)request.getAttribute("reviewInfo");   // 수정할 리뷰 정보

// url을 직접 입력해서 접근 시 로그인 한 회원정보와 일치하는지 검사
if (!reviewInfo.getMi_email().equals(memberInfo.getMi_email())) {
   out.println("<script>");
...
<div id="wrapper">
   ...
   <input type="hidden" name="idx" value="<%=reviewInfo.getPr_idx() %>" />
   <input type="hidden" name="oldImg1" value="<%=reviewInfo.getPr_img1() %>" />
   <input type="hidden" name="oldImg2" value="<%=reviewInfo.getPr_img2() %>" />
   <input type="hidden" name="oldImg3" value="<%=reviewInfo.getPr_img3() %>" />
   ...
   <td width="13%" style="padding-left:140px; padding-bottom:20px;">
      <img src="../product/pdt_img/<%=reviewInfo.getPi_img1() %>" width="100" height="100" />
   </td>
   <td width="*" colspan="2" valign="top" style="padding-left:40px;">
      <strong>[<%=reviewInfo.getBr_name() %>]&nbsp;<%=reviewInfo.getPi_name() %></strong><br />
      <%=reviewInfo.getBr_opt() %><br />
      <%=reviewInfo.getOd_pdtprice() %>원 ㅣ <%=reviewInfo.getOd_cnt() %>개
   </td>
   </tr>
   <tr>
   ...
   <% for (int j = arrStar.length ; j > 0 ; j--) { %>
      <option value="<%=j%>" <% if (reviewInfo.getPr_star() == j) { %>selected<% } %>><%=arrStar[j-1] %></option>
   <% } %>
   </select>
   </td>
   </tr>
   ...
   <td colspan="3">
      <textarea cols="122" rows="10" name="content" maxlength="500" onkeyup="chkContent(this.value);" placeholder="리뷰는 다른 고객에게 큰 도움이 됩니다."><%=reviewInfo.getPr_content()%></textarea>
      <br /><span id="contentMsg"></span>
   </td>
   </tr>
   <tr>
   <td colspan="2" style="padding-left:140px;">
   <% if (reviewInfo.getPr_img1() != null && !reviewInfo.getPr_img1().equals("")) { %>
      <img src="review_img/<%=reviewInfo.getPr_img1() %>" width="100" height="100" />
   <% } %>
   <% if (reviewInfo.getPr_img2() != null && !reviewInfo.getPr_img2().equals("")) { %>
      <img src="review_img/<%=reviewInfo.getPr_img2() %>" width="100" height="100" />
   <% } %>
   <% if (reviewInfo.getPr_img3() != null && !reviewInfo.getPr_img3().equals("")) { %>
      <img src="review_img/<%=reviewInfo.getPr_img3() %>" width="100" height="100" />
   <% } %>
   </td>
   <td align="right" style="padding-right:50px; padding-top:30px;"><!-- 이미지 미리보기? -->
